...
RESPONSE_TEMPLATES = [
    # תבנית 1 - קצרה וישירה
    """היי, ראיתי שאת/ה מחפש/ת עבודה.
יש לנו משרה של {job_title} באזור {location}.
אם מעניין שלח/י לי הודעה ואספר יותר""",

    # תבנית 2 - יותר אישית
    """שלום, נתקלתי בפוסט שלך.
יש לנו תפקיד פתוח של {job_title} ב{location}.
אם רלוונטי תכתוב/י לי בפרטי""",

    # תבנית 3 - מזדמן
    """היי, מחפשת משהו בתחום {job_title}?
יש לנו משרה באזור {location}.
תשלח/י הודעה אם מעניין""",

    # תבנית 4 - קצר מאוד
    """שלום,
יש משרה פתוחה ב{job_title} באזור {location}.
מעניין? כתוב/י לי בפרטי""",

    # תבנית 5 - יותר חמה
    """היי, אני מגייסת ל-AIG.
יש לי משרה של {job_title} ב{location} שאולי תתאים לך.
אם רוצה לשמוע עוד פרטים תכתוב/י לי"""
]
...
AUTOMATION_SETTINGS = {
    # מגבלות יומיות
    "max_responses_per_day": 10,  # מקסימום 10 תגובות ליום (שמרני)
    "max_responses_per_hour": 3,   # מקסימום 3 תגובות לשעה
    
    # עיכובים (בשניות)
    "delay_between_responses_min": 45,   # מינימום 45 שניות
    "delay_between_responses_max": 120,  # מקסימום 2 דקות
    "delay_between_groups_min": 30,      # 30 שניות בין קבוצות (לבדיקה)
    "delay_between_groups_max": 60,      # 1 דקה בין קבוצות (לבדיקה)
    
    # הגדרות סריקה
    "posts_to_scan_per_group": 50,  # כמה פוסטים לסרוק בכל קבוצה (50 = יותר סיכוי למצוא מועמדים)
...
BROWSER_SETTINGS = {
    ...
    "slow_mo": 500,     # האטה של 500ms בין פעולות (נראה אנושי יותר)
